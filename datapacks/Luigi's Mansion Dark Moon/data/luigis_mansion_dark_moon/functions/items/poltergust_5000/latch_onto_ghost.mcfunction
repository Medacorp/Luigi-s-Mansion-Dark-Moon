...
execute if entity @s[tag=!already_included] unless data entity @s data run data merge entity @s {ArmorItems:[{},{},{},{id:"minecraft:carved_pumpkin",Count:1b,tag:{attacked_by:[]}}]}
execute if entity @s[tag=!already_included] unless data entity @s data.attacked_by run data modify entity @s data merge value {attacked_by:[]}
execute if entity @s[tag=!already_included] in minecraft:overworld run data modify entity @s data.attacked_by append from block 0 0 0 Items[0].tag.Value
