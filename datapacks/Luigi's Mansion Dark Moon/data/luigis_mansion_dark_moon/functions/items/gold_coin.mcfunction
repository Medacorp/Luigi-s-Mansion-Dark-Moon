playsound luigis_mansion_dark_moon:item.gold_coin.obtain player @a[tag=same_room] ~ ~ ~ 1
function luigis_mansion_dark_moon:items/retreive_player_money/retreive
execute store result score #temp Time run data get storage luigis_mansion_dark_moon:data my_money.money.gold_coin
execute unless score #temp ActionTime matches 2147483647 store result storage luigis_mansion_dark_moon:data my_money.money.gold_coin int 1 run scoreboard players add #temp Time 1
