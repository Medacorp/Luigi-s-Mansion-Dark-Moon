# Rotate if moving
execute unless entity @s[tag=!open_door,tag=!locked_door,tag=!unlock_door] run scoreboard players reset @s PlayerRotation
...
execute unless entity @s[scores={RotationDif=-50000..50000},tag=!stop_model] run function luigis_mansion_dark_moon:animations/luigi/rotate_body
scoreboard players remove @s[scores={PlayerRotation=360000..}] PlayerRotation 360000
...
# Move with
execute rotated as @s run teleport @s[tag=!stop_model,tag=!sneak_pos] ^ ^1.65 ^-.25
execute rotated as @s run teleport @s[tag=!stop_model,tag=sneak_pos] ^ ^1.4 ^-.25

execute unless data storage luigis_mansion_dark_moon:data luigi{swimming:0b,gliding:0b} run teleport @s ~ ~-1 ~
execute unless data storage luigis_mansion_dark_moon:data luigi{swimming:0b,gliding:0b} if entity @s[tag=!stop_model] run data modify entity @s Rotation[1] set from entity @a[tag=this_luigi,limit=1] Rotation[1]
execute unless data storage luigis_mansion_dark_moon:data luigi{swimming:0b,gliding:0b} if entity @s[tag=!stop_model] run data modify entity @s Rotation[0] set from entity @a[tag=this_luigi,limit=1] Rotation[0]
execute if data storage luigis_mansion_dark_moon:data luigi{swimming:0b,gliding:0b} if entity @s[tag=!stop_model] run data modify entity @s Rotation[1] set value 0.0f